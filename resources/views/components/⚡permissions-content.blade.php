...
new class extends Component {
    ...
    public function savePermission(): void
    {
        ...
        $this->updateLoadedTargetPermission($target->id, $permission);
    ...
    private function loadTargets(): void
    {
        $this->departments = Department::query()
            ->orderBy('code')
            ->get(['id', 'code', 'name', 'permission']);
        $this->personTypes = PersonType::query()
            ->orderBy('name')
    ...
    private function updateLoadedTargetPermission(int $targetId, ?array $permission): void
    {
        $property = match ($this->targetType) {
            'department' => 'departments',
            'role' => 'personTypes',
            default => 'persons',
        };

        $this->{$property}->firstWhere('id', $targetId)?->forceFill(['permission' => $permission]);
    }
    ...
    {
        return $this->allowedDates !== [] || $this->excludedDates !== [] || $this->allowedTimes !== [] || $this->excludedTimes !== [] || $this->allowedWeekdays !== [] || $this->maxEntriesPerDay !== null || $this->maxEntries !== null || $this->maxDurationPerSession !== null || $this->maxDuration !== null || $this->constraintsJson === '{}';
    }
    ...
    private function validatedConstraints(): array
    {
        ...
        if (!is_array($constraints) || array_diff(array_keys($constraints), ['allowed_date', 'excluded_date', 'allowed_time', 'excluded_time', 'allowed_weekdays', 'max_entries_per_day', 'max_entries', 'max_duration_per_session', 'max_duration']) !== []) {
    ...
    private function validateRanges(array $ranges, string $key, string $format, string $errorField): void
    {
        foreach ($ranges as $range) {
            ...
            try {
                ...
                if ($start === false || $end === false || ($startErrors !== false && ($startErrors['warning_count'] > 0 || $startErrors['error_count'] > 0)) || ($endErrors !== false && ($endErrors['warning_count'] > 0 || $endErrors['error_count'] > 0)) || $start->format($format) !== $range['start'] || $end->format($format) !== $range['end'] || $start->greaterThan($end)) {
                    throw new \Exception();
                }
            } catch (\Throwable) {
...
<div class="flex min-h-screen flex-col overflow-y-auto bg-slate-100 p-3 sm:p-6 lg:h-screen">
    @if ($showErrorDialog && $errors->isNotEmpty())
        ...
        @endphp
        <div class="fixed inset-0 z-100 flex items-center justify-center bg-slate-950/70 p-4" role="dialog"
            aria-modal="true" aria-labelledby="permission-error-title">
            <div class="w-full max-w-lg rounded-lg border border-red-200 bg-white shadow-2xl" x-data="{
                trapFocus(event) {
                ...
                x-init="$nextTick(() => $el.querySelector('[autofocus]')?.focus())" x-on:keydown.tab="trapFocus($event)"
                ...
                <div class="flex items-start gap-3 border-b border-red-100 bg-red-50 px-5 py-4">
                    <div class="flex size-9 shrink-0 items-center justify-center rounded-full bg-red-100 text-lg font-bold text-red-700"
                ...
                <div class="flex justify-end border-t border-slate-200 px-5 py-4">
                    <button type="button" wire:click="closeErrorDialog" autofocus
                        class="rounded-md bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-500 focus:ring-offset-2">Đã
                        hiểu, sửa lỗi</button>
                </div>
            </div>
        </div>
    ...
    @if ($showSuccessDialog)
        <div class="fixed inset-0 z-100 flex items-center justify-center bg-slate-950/70 p-4" role="dialog"
            aria-modal="true" aria-labelledby="permission-success-title">
            <div class="w-full max-w-md rounded-lg border border-teal-200 bg-white shadow-2xl" x-data="{
                trapFocus(event) {
                    const focusable = [...$el.querySelectorAll('button:not([disabled]), [href], input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex=\'-1\'])')];
                    if (focusable.length === 0) return;
                    const first = focusable[0];
                ...
                <div class="flex flex-col items-center gap-3 px-5 py-7 text-center">
                    <div class="flex size-12 items-center justify-center rounded-full bg-teal-100 text-2xl font-bold text-teal-700"
                        aria-hidden="true">✓</div>
                    <div>
                        <h2 id="permission-success-title" class="font-semibold text-slate-900">Đã lưu quyền thành công
                        </h2>
                        <p class="mt-1 text-sm text-slate-500">Quyền ra vào đã được cập nhật cho đối tượng đã chọn.</p>
                    </div>
                </div>
                <div class="flex justify-end border-t border-slate-200 px-5 py-4">
                    <button type="button" wire:click="closeSuccessDialog" autofocus
                        class="rounded-md bg-teal-700 px-4 py-2.5 text-sm font-semibold text-white hover:bg-teal-800 focus:outline-none focus:ring-2 focus:ring-teal-600 focus:ring-offset-2">Đã
                        hiểu</button>
                </div>
            </div>
        </div>
    ...
    <div class="mx-auto flex min-h-0 w-full max-w-7xl flex-1 flex-col gap-4 sm:gap-5">
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-slate-900">Cấp quyền ra vào</h1>
            <p class="mt-1 text-sm text-slate-500">Chọn đơn vị, vai trò hoặc một người cụ thể rồi đặt điều kiện đi qua
                cổng.</p>
        </div>

        <form wire:submit="savePermission"
            class="grid min-h-0 flex-1 grid-rows-[auto_minmax(0,1fr)] gap-0 rounded-lg border border-slate-200 bg-white shadow-sm lg:overflow-hidden lg:grid-rows-[minmax(0,1fr)] lg:grid-cols-[minmax(18rem,0.72fr)_minmax(0,1.28fr)]">
            <section x-data="{
                search: '',
                targetType: @js($targetType),
                selectedId: @js($targetId),
                targets: {
                ...
                get activeTargets() {
                    return this.targets[this.targetType] ?? [];
                },
                get visibleTargets() {
                    ...
                    return this.activeTargets.filter((target) => {
                        const searchable = this.targetType === 'person' ?
                            `${target.code ?? ''} ${target.full_name ?? ''}` :
                ...
                targetName(target) {
                    return this.targetType === 'person' ? target.full_name : target.name;
                },
                setTargetType(type) {
                    this.targetType = type;
                    this.search = '';
                    $wire.set('targetType', type);
                },
                selectTarget(targetId) {
                    this.selectedId = targetId;
                    $wire.selectTarget(targetId);
                },
            }" x-effect="targetType = $wire.targetType; selectedId = $wire.targetId"
                @class([
                    'flex min-h-0 overflow-auto flex-col gap-4 bg-slate-50 p-4 sm:gap-5 sm:p-5 lg:border-r lg:border-slate-200',
                ...
                <div>
                    ...
                    <p class="mt-1 text-sm leading-5 text-slate-500">Mọi thiết lập bên phải sẽ được áp dụng cho lựa chọn
                        này.</p>
                </div>
                <div class="flex flex-col gap-2 text-sm font-medium text-slate-700">
                    ...
                    <div class="grid grid-cols-3 gap-1 rounded-md border border-slate-200 bg-white p-1">
                        @foreach (['department' => 'Đơn vị', 'role' => 'Vai trò', 'person' => 'Người'] as $type => $label)
                            <button type="button" x-on:click="setTargetType('{{ $type }}')"
                                class="rounded px-2 py-2 text-xs font-semibold transition"
                                :class="targetType === '{{ $type }}' ? 'bg-slate-900 text-white' :
                                    'text-slate-500 hover:bg-slate-100'">{{ $label }}</button>
                        @endforeach
                    </div>
                </div>
                ...
                <label class="flex flex-col gap-2 text-sm font-medium text-slate-700">
                    <span class="flex items-center justify-between gap-2">
                        <span
                            x-text="targetType === 'department' ? 'Chọn đơn vị' : (targetType === 'role' ? 'Chọn vai trò' : 'Chọn người')"></span>
                        <span class="text-xs font-normal text-slate-400"><span x-text="visibleTargets.length"></span>
                            kết quả</span>
                    </span>
                    <span class="relative">
                        <span class="pointer-events-none absolute inset-y-0 left-3 flex items-center text-slate-400"
                            aria-hidden="true">⌕</span>
                        <input x-model="search" type="search" placeholder="Tìm theo tên hoặc mã..."
                            class="w-full rounded-md border border-slate-300 bg-white py-2.5 pl-9 pr-3 text-sm text-slate-900 outline-none focus:border-teal-600 focus:ring-2 focus:ring-teal-600/20">
                    </span>
                </label>

                <div class="flex min-h-60 max-h-80 flex-1 flex-col gap-2 overflow-y-auto pr-1 lg:max-h-none"
                    aria-label="Danh sách đối tượng">
                    <template x-for="target in visibleTargets" :key="`${targetType}-${target.id}`">
                        <button type="button" x-on:click="selectTarget(target.id)"
                            class="flex w-full items-center gap-3 rounded-md border p-3 text-left transition"
                            :class="selectedId === target.id ? 'border-teal-600 bg-teal-50 ring-1 ring-teal-600' :
                                'border-slate-200 bg-white hover:border-teal-300 hover:bg-white'">
                            <span class="flex size-9 shrink-0 items-center justify-center rounded-md text-xs font-bold"
                                :class="selectedId === target.id ? 'bg-teal-700 text-white' : 'bg-slate-100 text-slate-600'"
                                x-text="targetName(target).charAt(0).toUpperCase()"></span>
                            <span class="min-w-0 flex-1">
                                <span class="block truncate text-sm font-semibold text-slate-800"
                                    x-text="targetName(target)"></span>
                                <span x-show="targetType !== 'role'"
                                    class="mt-0.5 block truncate text-xs text-slate-500" x-text="target.code"></span>
                            </span>
                            <span class="shrink-0 text-[11px] font-semibold"
                                :class="target.permission !== null ? 'text-teal-700' : 'text-slate-400'"
                                x-text="target.permission !== null ? 'Đã cấu hình' : 'Chưa cấp quyền'"></span>
                        </button>
                    </template>
                    <div x-show="visibleTargets.length === 0"
            ...
            <section x-data="{
                ...
                class="flex min-h-0 min-w-0 flex-col gap-4 p-4 sm:gap-5 sm:p-5 lg:overflow-auto">
                <div>
                    ...
                    <p class="mt-1 text-sm leading-5 text-slate-500">Tạo điều kiện bằng biểu mẫu. Bạn có thể kết hợp
                        nhiều loại điều kiện.</p>
                </div>
                ...
                <div @class([
                    ...
                    @foreach (['deny' => ['Từ chối tất cả', 'Không được qua cổng'], 'unrestricted' => ['Cho phép mọi lúc', 'Không giới hạn'], 'constrained' => ['Theo điều kiện', 'Thiết lập chi tiết']] as $value => [$label, $description])
                        <label
                            class="flex cursor-pointer flex-col gap-1 rounded-md border p-3 transition has-checked:border-teal-600 has-checked:bg-teal-50">
                            <span class="flex items-center gap-2 text-sm font-semibold text-slate-800"><input
                                    type="radio" wire:model.live="permissionMode" value="{{ $value }}"
                                    class="text-teal-700 focus:ring-teal-600">{{ $label }}</span>
                ...
                @if ($permissionMode === 'constrained')
                    <div @class([
                        'grid min-h-60 flex-1 gap-4 rounded-lg border-2 border-slate-400 bg-slate-50 p-2 sm:gap-5 sm:p-3 lg:overflow-y-auto xl:grid-cols-2',
                        'rounded-md ring-2 ring-red-400 ring-offset-2' => $this->hasErrorHighlight(
                            'constraints'),
                    ])>
                        <section @class([
                            'flex flex-col gap-3 rounded-md border border-slate-200 p-4',
                            'ring-2 ring-red-400 ring-offset-2' => $this->hasErrorHighlight(
                                'allowedDates'),
                        ])
                            x-bind:class="clientErrors.allowedDates ? 'ring-2 ring-red-400 ring-offset-2' : ''">
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    <h3 class="font-semibold text-slate-900">Ngày được phép</h3>
                                    <p class="text-xs text-slate-500">Chỉ cho phép trong các khoảng ngày này.</p>
                                </div>
                                <button type="button" wire:click="addRange('allowed_date')"
                                    class="shrink-0 rounded-md border border-green-200 px-2.5 py-1.5 text-xs font-semibold text-green-700 hover:border-green-500 hover:bg-green-100 hover:text-green-900">+
                                    Thêm khoảng</button>
                            </div>
                            @forelse ($allowedDates as $index => $range)
                                <div wire:key="allowed-date-{{ $index }}" data-range
                                    data-index="{{ $index }}"
                                    class="grid grid-cols-[1fr_1fr_auto] items-end gap-2">
                                    <label class="text-xs font-medium text-slate-600">Từ<input data-range-start
                                            inputmode="numeric" type="text"
                                            wire:model="allowedDates.{{ $index }}.start"
                                            x-on:input="formatInput($event, 'date'); validateRange('allowedDates', 'date', $el.closest('[data-range]'))"
                                            placeholder="dd/mm/yyyy"
                                            class="mt-1 w-full rounded-md border border-slate-300 px-2.5 py-2 text-sm text-slate-900"></label>
                                    <label class="text-xs font-medium text-slate-600">Đến<input data-range-end
                                            inputmode="numeric" type="text"
                                            wire:model="allowedDates.{{ $index }}.end"
                                            x-on:input="formatInput($event, 'date'); validateRange('allowedDates', 'date', $el.closest('[data-range]'))"
                                            placeholder="dd/mm/yyyy"
                                            class="mt-1 w-full rounded-md border border-slate-300 px-2.5 py-2 text-sm text-slate-900"></label>
                                    <button type="button" x-on:click="clientErrors.allowedDates = ''"
                                        wire:click="removeRange('allowed_date', {{ $index }})"
                                        aria-label="Xóa khoảng ngày"
                                        class="mb-1 rounded-md px-2 py-2 text-slate-400 hover:bg-red-50 hover:text-red-600">&times;</button>
                                </div>
                            @empty
                                <p class="rounded-md bg-slate-50 px-3 py-2 text-xs text-slate-500">Không giới hạn theo
                                    ngày.</p>
                            @endforelse
                            @error('allowedDates')
                                <span class="text-xs text-red-600">{{ $message }}</span>
                            @enderror
                            <span x-cloak x-show="clientErrors.allowedDates" x-text="clientErrors.allowedDates"
                        ...
                        <section @class([
                            ...
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    <h3 class="font-semibold text-slate-900">Ngày bị loại trừ</h3>
                                    <p class="text-xs text-slate-500">Không cho phép trong các khoảng ngày này.</p>
                                </div>
                                <button type="button" wire:click="addRange('excluded_date')"
                                    class="shrink-0 rounded-md border border-red-200 px-2.5 py-1.5 text-xs font-semibold text-red-700 hover:border-red-500 hover:bg-red-100 hover:text-red-900">+
                                    Thêm khoảng</button>
                            </div>
                            @forelse ($excludedDates as $index => $range)
                                <div wire:key="excluded-date-{{ $index }}" data-range
                                    data-index="{{ $index }}"
                                    class="grid grid-cols-[1fr_1fr_auto] items-end gap-2">
                                    <label class="text-xs font-medium text-slate-600">Từ<input data-range-start
                                            inputmode="numeric" type="text"
                                            wire:model="excludedDates.{{ $index }}.start"
                                            x-on:input="formatInput($event, 'date'); validateRange('excludedDates', 'date', $el.closest('[data-range]'))"
                                            placeholder="dd/mm/yyyy"
                                            class="mt-1 w-full rounded-md border border-slate-300 px-2.5 py-2 text-sm text-slate-900"></label>
                                    <label class="text-xs font-medium text-slate-600">Đến<input data-range-end
                                            inputmode="numeric" type="text"
                                            wire:model="excludedDates.{{ $index }}.end"
                                            x-on:input="formatInput($event, 'date'); validateRange('excludedDates', 'date', $el.closest('[data-range]'))"
                                            placeholder="dd/mm/yyyy"
                                            class="mt-1 w-full rounded-md border border-slate-300 px-2.5 py-2 text-sm text-slate-900"></label>
                                    <button type="button" x-on:click="clientErrors.excludedDates = ''"
                                        wire:click="removeRange('excluded_date', {{ $index }})"
                                        aria-label="Xóa khoảng ngày"
                                        class="mb-1 rounded-md px-2 py-2 text-slate-400 hover:bg-red-50 hover:text-red-600">&times;</button>
                                </div>
                            @empty
                                <p class="rounded-md bg-slate-50 px-3 py-2 text-xs text-slate-500">Không có ngày loại
                                    trừ.</p>
                            @endforelse
                            @error('excludedDates')
                                <span class="text-xs text-red-600">{{ $message }}</span>
                            @enderror
                            <span x-cloak x-show="clientErrors.excludedDates" x-text="clientErrors.excludedDates"
                                class="text-xs text-red-600"></span>
                        </section>
                        ...
                        @foreach ([['allowed_time', 'allowedTimes', 'Khung giờ được phép', 'Chỉ cho phép trong các khung giờ này.'], ['excluded_time', 'excludedTimes', 'Khung giờ bị loại trừ', 'Không cho phép trong các khung giờ này.']] as [$type, $property, $title, $description])
                            <section @class([
                                ...
                                <div class="flex items-start justify-between gap-3">
                                    ...
                                    <button type="button" wire:click="addRange('{{ $type }}')"
                                        class="shrink-0 rounded-md border {{ $type === 'allowed_time' ? 'border-green-200 text-green-700 hover:border-green-500 hover:bg-green-100 hover:text-green-900' : 'border-red-200 text-red-700 hover:border-red-500 hover:bg-red-100 hover:text-red-900' }} px-2.5 py-1.5 text-xs font-semibold">+
                                        Thêm khoảng</button>
                                </div>
                                @forelse ($$property as $index => $range)
                                    <div wire:key="{{ $type }}-{{ $index }}" data-range
                                        data-index="{{ $index }}"
                                        class="grid grid-cols-[1fr_1fr_auto] items-end gap-2">
                                        <label class="text-xs font-medium text-slate-600">Từ<input data-range-start
                                                inputmode="numeric" type="text"
                                                wire:model="{{ $property }}.{{ $index }}.start"
                                                x-on:input="formatInput($event, 'time'); validateRange('{{ $property }}', 'time', $el.closest('[data-range]'))"
                                                placeholder="hh:mm"
                                                class="mt-1 w-full rounded-md border border-slate-300 px-2.5 py-2 text-sm text-slate-900"></label>
                                        <label class="text-xs font-medium text-slate-600">Đến<input data-range-end
                                                inputmode="numeric" type="text"
                                                wire:model="{{ $property }}.{{ $index }}.end"
                                                x-on:input="formatInput($event, 'time'); validateRange('{{ $property }}', 'time', $el.closest('[data-range]'))"
                                                placeholder="hh:mm"
                                                class="mt-1 w-full rounded-md border border-slate-300 px-2.5 py-2 text-sm text-slate-900"></label>
                                        <button type="button" x-on:click="clientErrors.{{ $property }} = ''"
                                            wire:click="removeRange('{{ $type }}', {{ $index }})"
                                            aria-label="Xóa khung giờ"
                                            class="mb-1 rounded-md px-2 py-2 text-slate-400 hover:bg-red-50 hover:text-red-600">&times;</button>
                                    </div>
                                @empty
                                    <p class="rounded-md bg-slate-50 px-3 py-2 text-xs text-slate-500">Không giới hạn
                                        theo giờ.</p>
                                @endforelse
                                @error($property)
                                    <span class="text-xs text-red-600">{{ $message }}</span>
                                @enderror
                                <span x-cloak x-show="clientErrors.{{ $property }}"
                                    x-text="clientErrors.{{ $property }}" class="text-xs text-red-600"></span>
                            </section>
                        @endforeach
                        ...
                        <section @class([
                            ...
                            <div>
                                <h3 class="font-semibold text-slate-900">Ngày trong tuần</h3>
                                <p class="text-xs text-slate-500">Chọn ít nhất một ngày nếu muốn giới hạn lịch.</p>
                            </div>
                            <div class="grid grid-cols-4 gap-2 sm:grid-cols-7">
                                @foreach ([1 => 'CN', 2 => 'T2', 3 => 'T3', 4 => 'T4', 5 => 'T5', 6 => 'T6', 7 => 'T7'] as $day => $label)
                                    <label
                                        class="flex cursor-pointer flex-col items-center gap-1 rounded-md border border-slate-200 px-2 py-2 text-xs has-checked:border-teal-600 has-checked:bg-teal-50"><input
                                            type="checkbox" wire:model="allowedWeekdays" value="{{ $day }}"
                                            class="text-teal-700 focus:ring-teal-600"><span>{{ $label }}</span></label>
                                @endforeach
                            </div>
                            @error('allowedWeekdays')
                                <span class="text-xs text-red-600">{{ $message }}</span>
                            @enderror
                        </section>
                        ...
                        <section @class([
                            ...
                            <div class="col-span-2 xl:col-span-4">
                                <h3 class="font-semibold text-slate-900">Đặt giới hạn</h3>
                                <p class="mt-1 text-xs text-slate-500">Giới hạn số lượt vào hoặc thời gian ở lại trong
                                    khuôn viên.</p>
                            </div>
                            @foreach ([['maxEntriesPerDay', 'Lượt vào mỗi ngày'], ['maxEntries', 'Tổng lượt vào'], ['maxDurationPerSession', 'Thời lượng mỗi lượt (phút)'], ['maxDuration', 'Tổng thời lượng (phút)']] as [$field, $label])
                                <label
                                    class="flex flex-col gap-1 text-xs font-medium text-slate-600">{{ $label }}<input
                                        type="number" min="1" wire:model="{{ $field }}"
                                        x-on:input="validateNumber('limits', $event.target.value)"
                                        class="rounded-md border border-slate-300 px-2.5 py-2 text-sm text-slate-900"
                                        placeholder="Không giới hạn"></label>
                            @endforeach
                            @error('limits')
                                <span class="text-xs text-red-600 xl:col-span-4">{{ $message }}</span>
                            @enderror
                            <span x-cloak x-show="clientErrors.limits" x-text="clientErrors.limits"
                                class="text-xs text-red-600 sm:col-span-2 xl:col-span-4"></span>
                        </section>
                    </div>
                @endif

                @error('constraintsJson')
                    <span class="text-sm text-red-600">{{ $message }}</span>
                @enderror
                <div class="flex items-center justify-between gap-3">
                    @if (session('permissionSaved'))
                        <span class="text-sm font-medium text-teal-700">{{ session('permissionSaved') }}</span>
                    @else
                        <span></span>
                    @endif
                    <button type="submit"
                        class="rounded-md bg-teal-700 px-4 py-2.5 font-semibold text-white transition hover:bg-teal-800 focus:outline-none focus:ring-2 focus:ring-teal-600 focus:ring-offset-2">Lưu
                        quyền</button>
                </div>
            </section>
        </form>
