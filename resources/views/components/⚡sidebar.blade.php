    ...
    <div class="flex flex-col items-stretch h-screen min-w-50 w-[15vw] bg-gray-200">
        ...
        @foreach (config('tablist') as $tab)
            <a href="{{ route($tab['route']) }}"
                class="px-4 py-2 border-b-2 {{ request()->routeIs($tab['route']) ? 'border-blue-500 text-blue-500' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                {{ $tab['name'] }}
            </a>
        @endforeach
    </div>
